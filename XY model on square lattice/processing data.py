...
# load data
train_config1 = np.load('./raw data/spin_0-0.2.npy')
train_config2 = np.load('./raw data/spin_1.5-2.npy')
train_config = np.concatenate((train_config1, train_config2), axis=0)
train_T1 = np.load('./raw data/T_0-0.2.npy')
train_T2 = np.load('./raw data/T_1.5-2.npy')
train_T = np.concatenate((train_T1, train_T2), axis=0)

test_config = np.load('./raw data/spin_GT.npy')
test_T = np.load('./raw data/T_GT.npy')
...
for i in range(1,11):
    ...
    X, Y = np.meshgrid(np.arange(0, 16),np.arange(0, 16))
    U = np.cos(tmp_spin)
    V = np.sin(tmp_spin)
